Ext.define('eworker.view.User.UserRegistrationFormController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.user-userregistrationform',
    onAfterRender: async function(){
        await this.loadAccountTypes();
    },
    loadAccountTypes: async function(){
        let cmbstore = this.lookupReference('cmboAccountType');
        let response = await Ext.Ajax.request({ url: '/accounttypes', method: 'get' });
        if (response.responseText) {
            let records = JSON.parse(response.responseText);
            let store = Ext.create('Ext.data.Store', { data: records });
            cmbstore.setStore(store);
            store.load();
        }
